          ...
          $.ajax({
            ...
            success: function (data) {
              if (data) {
                if (data.status) {
                  $("#preview_photos").append(
                    '<div class="img-thumbnail"><img src="upload/photos/' +
                      data["thumb"] +
                      '" alt="' +
                      data["url"] +
                      '"><br><button class="btn btn-link remove_photo text-danger" title="' +
                      data["remove_title"] +
                      '">' +
                      data["remove"] +
                      '</button><input type="hidden" name="photos[]" value="' +
                      data["id"] +
                      '"></div>'
                  );
                } else {
              ...
              if (index === number_photos - 1) {
                ...
                $("#box_add_classified input[type=submit]").prop(
                  "disabled",
                  false
                );
                $progress_bar
                ...
                $progress_bar
                  .css("width", progress_bar_value + "%")
                  .attr("aria-valuenow", progress_bar_value)
                  .text(progress_bar_value + "%");
              }
            },
...
angular
  .module("addClassified", [])
  .controller("addClassified", function ($scope, $http, $q) {
    ...
    $scope.loadCategories = function (
      ...
      for (let i = 0; i < $scope.list_categories.length; i++) {
        if ($scope.list_categories[i].category_id == select_category_id) {
          $scope.list_categories.splice(i + 1, $scope.list_categories.length);
          break;
        }
      }
      ...
      $http(request).then(function (response) {
          ...
          $.each(response.data.options, function (index, value) {
            ...
            if (list_options[id] != undefined) {
              if (response.data.options[index].kind == "number") {
                response.data.options[index].value = parseInt(
                  list_options[id][0]
                );
              ...
              response.data.options[index].value =
                $scope.list_options[index].value;
            } else {
              response.data.options[index].value = "";
            }
          });
        ...
        if (
          !$.isEmptyObject(response.data.categories) &&
          (category_id > 0 || $scope.list_categories.length == 0)
        ) {
          selectCategory = "";
