				...
				<div class="col-md-7 col-sm-5 overflow_hidden">
					...
					{% if classified.location_name or classified.category_name %}
						<p class="main-color-1 small">
							{% if classified.location_name %}<a href="{{ path('classifieds') }}?location={{ classified.location_slug }}" title="{{ 'Location'|trans }}: {{ classified.location_name }}" class="main-color-1">{{ classified.location_name }}</a>{% endif %}
							{% if classified.location_name and classified.category_name %} | {% endif %}
							{% if classified.category_name %}
								<a href="{{ path('classifieds') }}?category={{ classified.category_id }}" title="{{ 'Category'|trans }}: {{ classified.category_name }}" class="main-color-2">{{ classified.category_name }}</a>
							{% endif %}
						</p>
					{% endif %}
					<p class="text-muted" itemprop="disambiguatingDescription">{{ classified.description|striptags|slice(0,220) }}{% if classified.description|striptags %}...{% endif %}</p>
				</div>
				...
				{% elseif controller=='clipboard' %}
					<div class="col-md-12">
						<form method="post">
							...
							<input type="submit" value="{{ 'Remove from clipboard'|trans }}" class="btn btn-danger btn-sm">
						</form>
					</div>
				{% endif %}
			</div>
