...
if(!_ADMIN_TEST_MODE_ and $admin->is_logged() and isset($_POST['data'])){
	...
	}elseif($post['action']=='position_location' and isset($post['id']) and isset($post['position']) and isset($post['plusminus']) and checkToken('admin_position_location', $post['token'])){
		setPosition('location',$post['id'],$post['position'],$post['plusminus']);
	}elseif($post['action']=='arrange_location_alphabetically' and checkToken('admin_arrange_location_alphabetically', $post['token'])){
		arrangeAlphabetically('location');
	}
}else{
	die('Access denied!');
}
