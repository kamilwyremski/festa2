...
class photo
{
	...
	public static function add()
	{
		...
		if (!in_array($path_parts['extension'], ['jpg', 'jpeg', 'png'])) {
			...
			$tmp = imagecreatetruecolor($newwidth, $newheight - 200);
			if ($path_parts['extension'] == "png") {
				imagesavealpha($tmp, true);
				$color = imagecolorallocatealpha($tmp, 0, 0, 0, 127);
				imagefill($tmp, 0, 0, $color);
			}
			imagecopyresampled($tmp, $src, 0, 0, $x, $y + 100, $newwidth, $newheight, $smallestSide, $smallestSide);
			$thumb = explode('.', $url)[0] . '_thumb.' . $path_parts['extension'];

			if ($path_parts['extension'] == "jpg" || $path_parts['extension'] == "jpeg") {
				imagejpeg($tmp, _FOLDER_PHOTOS_ . $folder . $thumb, $settings['photo_quality']);
				static::correctImageOrientation(_FOLDER_PHOTOS_ . $folder . $thumb);
			} else {
			...
			return ['status' => 1, 'id' => $id, 'url' => $folder . $url, 'thumb' => $folder . $thumb, 'remove_title' => trans('Remove photo') . ': ' . $url, 'remove' => trans('Remove')];
		}
	}
		...
		if (function_exists('exif_read_data')) {
			...
			if ($exif && isset($exif['Orientation'])) {
				...
				if ($orientation != 1) {
					...
					imagejpeg($img, $filename, $settings['photo_quality']);
				}
			}
		}
	}
