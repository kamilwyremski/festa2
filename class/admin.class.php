...
class admin {
	...
	public function login(array $data){
		...
		if($sth->rowCount()<5){
			...
			$sth = $db->prepare('SELECT '._DB_PREFIX_.'admin.id, username, password FROM '._DB_PREFIX_.'admin_session, '._DB_PREFIX_.'admin WHERE username=:username AND code=:code AND ip=:ip LIMIT 1');
	...
	public function getLogs(){
		...
		$limit = 100;
		$admin_logs = [];
		$sth = $db->query('SELECT SQL_CALC_FOUND_ROWS * FROM '._DB_PREFIX_.'admin_logs ORDER BY '.orderBy().' LIMIT '.paginationPageFrom($limit).','.$limit.'');
