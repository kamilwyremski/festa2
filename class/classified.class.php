...
class classified
{
	...
	public static function list(int $limit = 10, array $data = [], string $type = 'all')
	{
		...
		if (!empty($data['location'])) {
			$where_statement .= ' AND c.location_id = (SELECT id FROM ' . _DB_PREFIX_ . 'location WHERE slug=:location LIMIT 1) ';
			$bind_values['location'] = $data['location'];
		}
		if (!empty($data['state'])) {
		...
		if ($type == 'classifieds' or $type == 'my_classifieds' or $type == 'clipboard' or $type == 'index_page') {
			$select .= ', cat.name AS category_name, l.name AS location_name, l.slug AS location_slug';
			$join .= 'LEFT JOIN ' . _DB_PREFIX_ . 'category cat ON c.category_id = cat.id
			LEFT JOIN ' . _DB_PREFIX_ . 'location l ON c.location_id = l.id';
		}
		if ($type != 'admin') {
	...
	public static function edit(int $id, array $data, bool $checkCost = false)
	{
		...
		$sth = $db->prepare('UPDATE ' . _DB_PREFIX_ . 'classified SET name=:name, slug=:slug, address=:address, phone=:phone, email=:email, category_id=:category_id, state_id=:state_id, state2_id=:state2_id, location_id=:location_id, description=:description, address_lat=:address_lat, address_long=:address_long WHERE id=:id LIMIT 1');
		...
		$sth->bindValue(':location_id', $data['location_id'], PDO::PARAM_INT);
	...
	public static function activate(int $id, string $date_finish = '', bool $admin_confirmed = false)
	{
		global $db, $settings;
		if (!$date_finish) {
			$classified = classified::show($id, 'payment');
			if ($classified['duration_id']) {
				$duration = duration::getDays($classified['duration_id']);
				$date_finish = date('Y-m-d', strtotime(date("Y-m-d") . ' + ' . $duration['length'] . ' days'));
			} else {
	...
	public static function listSimilar(array $classified, int $limit = 10)
	{
		...
		if ($classified['category_id']) {
			$data['category_id'] = $classified['category_id'];
		}
		if ($classified['location_id']) {
			$data['location_id'] = $classified['location_id'];
		}
		$data['not_id'] = $classified['id'];

		$similar_classifieds = static::list($limit, $data, 'index_page');

		return $similar_classifieds;
	}
