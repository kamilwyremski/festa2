...
if(_CLASSIFIEDS_PATH_ and (isset($_GET['state']) or isset($_GET['category']) or isset($_GET['location']))){
	$redirect_path = '';
	if(isset($_GET['state'])){
	...
	if(isset($_GET['location'])){
		if($_GET['location']){$redirect_path .= '/'._PREFIX_LOCATION_.$_GET['location'];}
		unset($_GET['location']);
	}
...
if(!empty($_GET['path'])){
	...
	if(substr($path_parts[0], 0, strlen(_PREFIX_STATE_))==_PREFIX_STATE_){
		...
		if(isset($path_parts[0]) and substr($path_parts[0], 0, strlen(_PREFIX_CATEGORY_))!=_PREFIX_CATEGORY_ and substr($path_parts[0], 0, strlen(_PREFIX_LOCATION_))!=_PREFIX_LOCATION_){
	...
	if(isset($path_parts[0]) and substr($path_parts[0], 0, strlen(_PREFIX_CATEGORY_))==_PREFIX_CATEGORY_){
		...
		foreach($path_parts as $category_path){
			if(substr($category_path, 0, strlen(_PREFIX_LOCATION_))!=_PREFIX_LOCATION_){
				$_GET['category_path'] .= '/'.array_shift($path_parts);
			}
		}
	}

	if(isset($path_parts[0]) and substr($path_parts[0], 0, strlen(_PREFIX_LOCATION_))==_PREFIX_LOCATION_){
		$_GET['location'] = substr(array_shift($path_parts), strlen(_PREFIX_LOCATION_));
	}
}

if(!empty($_GET['path']) and $_GET['path']!=$links['classifieds'] and !isset($_GET['state']) and !isset($_GET['category_path']) and !isset($_GET['location'])){
	throw new noFoundException();
}
...
if(!empty($_GET['location'])){
	$location = location::showBySlug($_GET['location']);
	if(empty($location)){
		throw new noFoundException();
	}
}
...
if($settings['show_breadcrumbs']){
	...
	if(!empty($location)){array_push($breadcrumbs , ['path'=>$path.'&location='.$location['slug'], 'name'=>$location['name']]);}
...
if(!empty($location)){
  if($settings['h1']){
		$settings['h1'] .= ' '.makeFirstLetterSmall($location['name']);
  }else{
		$settings['h1'] .= $location['name'];
  }
  if($settings['seo_title']){
		$settings['seo_title'] .= ' '.makeFirstLetterSmall($location['name']);
  }else{
		$settings['seo_title'] .= $location['name'];
  }
}
...
if(!empty($categories)){
	...
	if(!empty($location)){$path .= '&location='.$location['slug'];}

	if(isset($_GET['search'])){
		$gets_array = $_GET;
		unset($gets_array['path'],$gets_array['state'],$gets_array['state2'],$gets_array['category'],$gets_array['location'],$gets_array['page'],$gets_array['sort'],$gets_array['category_path'],$gets_array['slug']);
